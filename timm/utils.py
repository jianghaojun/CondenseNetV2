...
class CheckpointSaver:
    ...
    def save_checkpoint(self, model, optimizer, args, epoch, model_ema=None, metric=None, use_amp=False):
        ...
        # if mox.file.exists(last_save_path):
        #     mox.file.remove(last_save_path) # required for Windows support.
        # mox.file.rename(tmp_save_path, last_save_path)
        ...
        if (len(self.checkpoint_files) < self.max_history
            ...
            # mox.file.copy(last_save_path, save_path)
            ...
            if metric is not None and (self.best_metric is None or self.cmp(metric, self.best_metric)):
                ...
                # if mox.file.exists(best_save_path):
                #     mox.file.remove(best_save_path)
                # mox.file.copy(last_save_path, best_save_path)
    ...
    def update(self, model):
        ...
        with torch.no_grad():
            ...
            for k, ema_v in self.ema.state_dict().items():
                ...
                ### Modified by hjjiang 2020.9.17 for model
                # Modification:
                # 1. Add IF-ELSE, the reason is:
                #   Efrnet _mask's element should be binary, thus, we should not apply moving average on _mask.
                if k.split('.')[-1] in ['_mask', '_count', '_stage']:
                    ema_v.copy_(model_v)
                    # logging.info("Model ema copy {}".format(k.split('.')[-1]))
                ### End modification
                else:
                    ema_v.copy_(ema_v * self.decay + (1. - self.decay) * model_v) # Original code
